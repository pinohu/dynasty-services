# App Hosting (Vercel + 20i) — Business-in-a-Box
...
- **Stripe Product:** prod_dynasty_host_app
...
1. **create vercel project** — {"action":"create_vercel_project","framework":"{{framework}}"}
2. **configure github cicd** — {"action":"configure_github_cicd"}
3. **setup custom domain** — {"action":"setup_custom_domain"}
4. **configure ssl** — {"action":"configure_ssl"}
5. **setup monitoring** — {"action":"setup_monitoring"}
6. **configure preview deployments** — {"action":"configure_preview_deployments"}
...
- [ ] **Cicd Guide** — Branded PDF delivered to client
- [ ] **Monitoring Guide** — Branded PDF delivered to client
...
### n8n Workflow: App Hosting (Vercel + 20i) Onboarding
...
**Solution:** Dynasty Empire built a comprehensive app hosting (vercel + 20i) system using production-grade infrastructure.
